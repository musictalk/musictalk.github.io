...
stateCmd s =
    case s of
        GotToken t ->
            Spotify.getUserInfo t

        LoggedIn _ _ ->
            Cmd.none

        Unlogged ->
            Debug.crash "Unlogged in state cmd"
...
pageCmd token model =
    case always model.page (Debug.log "pageCmd" (dumpModel model)) of
        IndexData _ ->
            [ Spotify.getPlaylists token ]

        PlaylistReq uid pid song ->
            let
                needFetchTracks =
                    case model.page of
                        PlaylistDetails playlist _ ->
                            playlist.id /= pid

                        _ ->
                            True
            in
                if Debug.log "needFetchTracks" needFetchTracks then
                    [ Spotify.getPlaylistTracks token uid pid ]
                else
                    []

        PlaylistDetails pl (Just songId) ->
            [ loadSongComments <|
                (,,,)
                    (pl.id ++ "/" ++ songId)
                    (Views.playlistSongUrl pl songId)
                    songId
                    1
            ]
...
urlUpdate result model =
    case fst <| Debug.log "urlUpdate/model" ( result, dumpModel model ) of
        ...
        Ok page ->
            case model.state of
                Unlogged ->
                    Debug.crash "unlogged" model

                LoggedIn token _ ->
                    let
                        m =
                            { model | page = Routing.pageToData model page }
                    in
...
update msg model =
    let
        _ =
            Debug.log "update/model" (dump msg model)
    in
        case msg {- Debug.log "update" msg -} of
            ...
            ReceiveTracks (Ok pl) ->
                let
                    _ =
                        Debug.log "ReceiveTracks model" model
                in
                    case model.page of
                        PlaylistReq _ _ songId ->
                            let token = case model.state of
                                    GotToken t -> t
                                    LoggedIn t _ -> t
                                    _ -> Debug.crash "should have a token"
                                model = { model | page = PlaylistDetails pl songId }
                            in
                                model ! pageCmd token model
